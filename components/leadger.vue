...
Apply filter
 ...
 <template #loading> Loading customers data. Please wait. </template>
      ...
      <Column v-if="visibleColumns.includes('CTRCODE')" sortable field="CTRCODE" header="CTR Code">
          <template #body="{ data }">
...
const formatDater = (date) => {
  ...
  if (ledgerResponseData.value?.metaData?.DATA) {
      storedData.value = ledgerResponseData.value.metaData.DATA.filter(item => {
          const voucherDate = item[5] ? new Date(item[5]) : null;
          return voucherDate && voucherDate >= startDate && voucherDate <= today;
      }).map(item => ({
          clientcode: item[0], COCD: item[4], voucherdate: item[5] ? new Date(item[5]) : null,  NARRATION: item[7], DR_AMT: item[8], CTRCODE:item[3],
          fromDate:item[1], todate:item[2], voucherdaten:item[6], cr_amt:item[9], openbal:item[10],  etrydatetime:item[11]
...
{ field: 'CTRCODE', header: 'CTR code' },
{ field: 'voucherdate', header: 'VOUCHERDATE' },
{ field: 'NARRATION', header: 'NARRATION' },
{ field: 'DR_AMT', header: 'DR Amt' },


]);

const selectedColumns = ref(columns.value.filter(col => !['NARRATION'].includes(col.field)));
