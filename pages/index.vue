  ...
  <div class="bg-surface-0 dark:bg-surface-950 px-6 py-20 md:px-12 lg:px-20">
    <div class="flex flex-wrap shadow">
      <div class="w-full lg:w-6/12 px-0 py-6 lg:p-16 bg-primary-50">
        <svg class="mb-12 ml-4 lg:ml-0 fill-surface-500" width="140" height="36" viewBox="0 0 140 36" fill="none"
        ...
        <Carousel :value="features">
          <template #item="slotProps">
            <div class="text-center mb-20 flex flex-col items-center">
              <img
                :src="'https://fqjltiegiezfetthbags.supabase.co/storage/v1/render/image/public/block.images/blocks/illustration/' + slotProps.data.image"
                alt="Image" class="mb-12 w-6/12" />
      ...
      <div 
        class="w-full flex justify-center flex-col items-center lg:w-6/12 p-6 lg:p-16 bg-surface-0 dark:bg-surface-900">
       
        <form @submit.prevent="sendClientCode" v-if="clientlogin">
         
          <label for="clientcode" class="block text-surface-900 dark:text-surface-0 font-medium mb-2">CLIENT
            CODE</label>
          <InputText  type="text" v-model="clientcode"  @input="formatInput" placeholder="Enter a client code" class="!w-full !mb-4 !p-4" />

          <Button label="Sign In" type="submit" class="w-full py-4 font-medium" />
        </form>

        <form class="w-full" @submit.prevent="otpsend" v-if="otpbox">
        
          <div class="card flex justify-center flex-col items-center">
            <label for="otp" class="block text-slate-500 dark:text-surface-0 font-medium mb-2">Enter your OTP</label>
            <InputOtp v-model="value" variant="filled" :length="6" size="large" />
            <Button label="Submit" type="submit" class=" py-2 mt-2 font-medium" />
          </div>
...
import { useAlphanumericInput } from '@/composables/useAlphanumericInput';
...
const { clientcode, formatInput } = useAlphanumericInput();
...
const sendClientCode = async () => {
  ...
  const apiUrl = 'https://backoffice.w3webtechnologies.co.in/bo-api/api-initiate-otp.php';
  ...
  // Ensure clientcode is defined (assuming it's a reactive reference)
  if (!clientcode.value) {
    toast.add({ severity: 'error', summary: 'Error Message', detail: 'Client code is required', group: 'br', life: 3000 });
    return;
  }
...
const otpsend = async () => {
  ...
  const apiUrl = 'https://backoffice.w3webtechnologies.co.in/bo-api/api-validate-otp.php';
  const authorization = 'F2CB3616F1EC269F0BF328CB77FEE4EFCDF5450D7BD21A94721C2F4E49E88F83A4FCE196070903C1BDCAA25F08F037538567D785FC56D139C09A6EC7927D5EFE';

  // Ensure clientcode is defined (assuming it's a reactive reference)
  if (!value.value) {
    toast.add({ severity: 'error', summary: 'Error Message', detail: 'OTP is required', group: 'br', life: 3000 });
    return;
  }

  const formData = new FormData();
  formData.append('clientCode', localStorage.getItem('clientcode'));
  formData.append('otpCode', value.value);
  formData.append('token', otptoken.value);
  ...
  try {
    ...
    const data = await response.json();
    if(data.status=='success'){
      toast.add({ severity: 'success', summary: 'Success Message', detail: data.message, group: 'br', life: 3000 });
